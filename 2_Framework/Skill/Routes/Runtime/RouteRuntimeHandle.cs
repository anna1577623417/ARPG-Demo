...
public sealed class RouteRuntimeHandle : IRouteRuntimeHandle
{
    ...
    public RouteRuntimeHandle(Player owner, SkillRouteDefinition def, SkillRouteRuntime rt,
        SkillEntrySlot slot, string keyLabel, SkillEntryService service)
    {
        _owner = owner;
        _def = def;
        _runtime = rt;
    ...
    public Sprite Icon
    {
        get
        {
            if (_runtime is MultiStageRouteRuntime ms && ms.TryGetHudIcon(out var stageIcon) && stageIcon != null)
            {
                return stageIcon;
            }

            return _def != null ? _def.Icon : null;
        }
    }
