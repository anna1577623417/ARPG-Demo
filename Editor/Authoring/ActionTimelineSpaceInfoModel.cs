...
internal readonly struct ActionTimelineSpaceInfoModel
{
    ...
    public static ActionTimelineSpaceInfoModel Build(in ActionTimelinePreviewContext ctx)
    {
        ...
        var world = ctx.HasMotionProfile
            ? ctx.MotionWorldPosition
            : origin + Quaternion.LookRotation(
                ctx.PlanarForward.sqrMagnitude > 0.0001f ? ctx.PlanarForward : Vector3.forward,
                Vector3.up) * local;
        var heading = ctx.UsesActionYawPreview
            ? ctx.ActionYawForward
            : ctx.HasCaptureOrigin
        ...
        var headingLabel = ctx.UsesActionYawPreview
            ? $"Heading:    ActionYaw ({yaw:F0}°)"
